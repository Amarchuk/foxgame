...
function createButtons() {
    for (var a = 0; 4 > a; a++) createButton(onTileClick, a);
    goButton = ui.add(game.add.button(1050, 600, "item", playLevel, this, 20, 21, 22));
    game.add.button(1160, 600, "item", resetLevel, this, 25, 26, 27)
}

function createButton(a, b) {
    var c, d = 1050, e = 50, f = 20, g = 90 + d, h = 90 * b + e + f * b, i = 5 * b, j = i + 1, k = i + 2;
    c = ui.add(game.add.button(g, h, "item", a, this, i, j, k)), c.spriteNumber = b
}

function onTileClick(a) {
    if (isPlaying !== !0) {
        orders.length >= 32;
        var b = 5 * a.spriteNumber + 2, c = b + 1, d = b + 1,
            e = game.add.button(0, 0, "item", onOrderClick, this, b, c, d);
        e.inputEnabled = !0, e.spriteNumber = a.spriteNumber, e.inactive = !1, orders.push(e), renderOrders()
    }
}

function onOrderClick(a) {
    isPlaying !== !0 && (orders.splice(a.index, 1), a.kill(), renderOrders())
}

function renderOrders() {
    for (var a = 0; a < orders.length; a++) {
        var b = orders[a], c = 5 * b.spriteNumber + 3, d = c + 1;
        b.inactive === !1 ? b.setFrames(d, c, d) : b.setFrames(c, d, d), b.y = 580, b.x = 25 + 40 * a, a > 22 && (b.y += 50, b.x -= 920), a > 44 && (b.y += 50, b.x -= 1800), b.index = a
    }
}

function stopButton() {
    resetDoors(90, 229, 900), resetDoors(110, 230, 1100), resetDoors(130, 231, 1300), isPlaying = !1;
    for (var a = 0; a < orders.length; a++) orders[a].inactive = !0;
    "undefined" != typeof mouseTween && mouseTween.stop(), executeOrder(), parseEntities();
    for (var a = 0; a < orders.length; a++) orders[a].inactive = !1;
    renderOrders()
}
...
function playLevel() {
    return isPlaying === !0 ? (goButton.setFrames(20, 21, 22), stopButton(), void 0) : (isPlaying = !0, goButton.setFrames(22, 22, 22), executeOrder(), void 0)
}

function resetLevel() {
    isPlaying === !0 && stopButton();
    for (var a = orders.length, b = a - 1; b >= 0; b--) onOrderClick(orders[b]);
...
function executeOrder() {
    if (isPlaying !== !1) {
        group.sort();
        for (var a, b = 0; b < orders.length; b++) orders[b].inactive === !1 && "undefined" == typeof a && (orders[b].inactive = !0, a = orders[b]);
        if ("undefined" != typeof a) {
            switch (a.spriteNumber) {
                case 0:
                    moveMouse("UP");
                    break;
                case 1:
...
function moveMouse(a) {
    var b = {x: mouse.x, y: mouse.y}, c = !0;
    "LEFT" === a && (b.x -= TILE_SIZE), "RIGHT" === a && (b.x += TILE_SIZE), "UP" === a && (b.y -= TILE_SIZE), "DOWN" === a && (b.y += TILE_SIZE);
    for (var d, e = Math.floor(b.x / TILE_SIZE), f = Math.floor(b.y / TILE_SIZE), g = 0; g < map.layers.length; g++) "entities" === map.layers[g].name && (d = map.getTile(e, f, g)), 6 === map.getTile(e, f, g) && (d = TILE_HOLE);
    return d === TILE_HOLE && (game.add.tween(mouse).to({angle: -125}, 1500, Phaser.Easing.Out, !0, 0, !1), game.add.tween(mouse.scale).to({
        x: 0,
        y: 0,
        angle: 125
    }, 1500, Phaser.Easing.In, !0, 0, !1), c = !1), 225 === d && toggleDoor(90, 229, 900), 226 === d && toggleDoor(110, 230, 1100), 227 === d && toggleDoor(130, 231, 1300), -1 !== TILE_WALLS.indexOf(d) ? (game.add.tween(mouse.scale).to({
        x: 1.1,
        y: 1.1
    }, 150, Phaser.Easing.Back.In, !0, 0, !1).to({
        x: 1,
        y: 1
    }, 150, Phaser.Easing.Back.Out, !0, 0, !1), game.add.tween(mouse).onCompleteCallback(executeOrder).start(), void 0) : (mouseTween = game.add.tween(mouse).to({
        x: b.x,
        y: b.y
    }, 150, Phaser.Easing.Back.InOut, !0, 0, !1).onCompleteCallback(function () {
        return -1 !== TILE_GOAL.indexOf(d) ? (victoryAnimation(), void 0) : 212 === d ? (executeOrder(), void 0) : (c && moveMouse(a), void 0)
    }), void 0)
}

function victoryAnimation() {
    mouseTween = game.add.tween(mouse.scale).to({x: 1.1, y: 1.1}, 150, Phaser.Easing.Back.In, !0, 0, !1).to({
        x: 1,
        y: 1
    }, 150, Phaser.Easing.Back.Out, !0, 0, !1).repeat(4);
    var a = this.game.state.states.levelselector.level;
    localStorage["level-" + a] = !0, game.add.tween(mouse).delay(500).onCompleteCallback(function () {
        "level8" === a ? this.game.state.start("ending", !0, !1) : goToLevelSecletor()
    }).start()
}

function goToLevelSecletor() {
    this.game.state.start("levelselector", !0, !1)
}

function resetDoors(a, b, c) {
    group.forEach(function (d) {
        if (d.name === c) for (var e = Math.floor(d.x / TILE_SIZE), f = Math.floor(d.y / TILE_SIZE), g = 0; g < map.layers.length; g++) "entities" === map.layers[g].name && (map.putTile(b, e, f, g), d.revive(), d.name = a)
    }, this)
}

function toggleDoor(a, b, c) {
    group.forEach(function (d) {
        var e = Math.floor(d.x / TILE_SIZE), f = Math.floor(d.y / TILE_SIZE);
        if (d.name === a) {
            for (var g = 0; g < map.layers.length; g++) "entities" === map.layers[g].name && map.putTile(c, e, f, g);
            d.name = c, d.kill()
        } else if (d.name === c) {
            for (var g = 0; g < map.layers.length; g++) "entities" === map.layers[g].name && map.putTile(b, e, f, g);
            d.name = a, d.revive()
        }
    }, this)
}
...
function createMap() {
    ...
    var a = this.game.state.states.levelselector.level;
    if (null === a) return this.game.state.start("levelselector"), void 0;
    map = game.add.tilemap(a), tileset = game.add.tileset("tiles"), decorations = game.add.tileset("decorations"), group = game.add.group();
    for (var b = 0; b < map.layers.length; b++) "entities" === map.layers[b].name ? mapLayer = map.layers[b] : game.add.tilemapLayer(0, 0, 1280, 720, tileset, map, b);
    parseEntities()
}

function parseEntities() {
    group.destroy(), group = game.add.group(), "undefined" != typeof scientist && scientist.kill(), "undefined" != typeof mouse && mouse.kill(), resetDoors();
    for (var a = 0; a < mapLayer.data.length; a++) for (var b = 0; b < mapLayer.data[a].length; b++) {
        var c = mapLayer.data[a][b];
        if (0 !== c) switch (c) {
            case 201:
                addWall(c, b, a);
                break;
...
function addOtherItem(a, b, c) {
    var d, e = [];
    e[217] = "bottle_blue", e[218] = "bottle_green", e[219] = "bottle_red", e[220] = "bottle_yellow", e[221] = "key", e[222] = "pill", e[223] = "pipette", e[224] = "pipetteset";
    var f = [];
    if (f[225] = RED_BUTTON, f[229] = RED_DOOR, f[226] = GREEN_BUTTON, f[230] = GREEN_DOOR, f[227] = YELLOW_BUTTON, f[231] = YELLOW_DOOR, "undefined" != typeof f[a]) {
        var g = group.create(b * TILE_SIZE, c * TILE_SIZE, "tiles-sprites", f[a]);
        return g.name = f[a], void 0
    }
    if ("undefined" == typeof e[a]) return console.warn(a, "is undefined"), void 0;
    var h = group.add(game.add.sprite(b * TILE_SIZE + TILE_SIZE / 2, c * TILE_SIZE + TILE_SIZE / 2, "tiles-sprites", e[d]));
    h.body.immovable = !0
}
...
function addPlayer(a, b, c) {
    mouse = group.add(game.add.sprite(b * TILE_SIZE + TILE_SIZE / 2, c * TILE_SIZE + TILE_SIZE / 2, "monkey")), mouse.anchor.setTo(.5, .9)
}

function addGoal(a, b, c) {
    var d = group.create(b * TILE_SIZE, c * TILE_SIZE, "tile-logic", a - 201);
    d.body.immovable = !0
}

function clearSaves() {
    for (var a = 1; 15 >= a; a++) localStorage["level-level1"] = null
}

function playMusic(a) {
    if ("undefined" == typeof menuMusic && (menuMusic = game.add.audio("menu-music", 1, !0)), "undefined" == typeof gameMusic && (gameMusic = game.add.audio("game-music", 1, !0)), a === !0) {
        if (menuMusic.isPlaying === !0) return;
        gameMusic.pause(), menuMusic.play("", 0, 1, !0)
    } else {
        if (gameMusic.isPlaying === !0) return;
        menuMusic.pause(), gameMusic.play("", 0, 1, !0)
    }
}

var MouseGame = {}, orders = [], mouseTween, isPlaying = !1, goButton;
!function (a, b) {
    ...
    c.InteractionManager = function () {
        ...
        var d = (a.worldAlpha, a.graphicsData.length);
        if (0 !== d) {
        ...
        var b = this.context, c = a.verticies, d = (a.uvs, c.length / 2);
        ...
        for (var a, b, d, e, f, g, h, i, j, k, l, m, n, o, p, q, r = (this.gl, 0), s = this.head, t = this.verticies, u = this.uvs, v = this.colors; s;) {
        ...
        if (a = a || 0, void 0 == b && (b = this.size), this.dirty && (this.refresh(), this.dirty = !1), 0 !== this.size) {
            ...
            var f = b - a;
            d.drawElements(d.TRIANGLES, 6 * f, d.UNSIGNED_SHORT, 2 * a * 6)
        }
    ...
    return c.WebGLRenderGroup.prototype.handleFilterBlock = function (a, b) {
        ...
        var e = c.stripShader, f = (e.program, c.mat3.clone(a.worldTransform));
        ...
        var d = this.gl, e = (c.shaderProgram, a.tilePosition), f = a.tileScale, g = e.x / a.texture.baseTexture.width,
        ...
        this.game = a, this.layers = null, "string" == typeof b ? (this.key = b, console.log(a, b), this.layers = a.cache.getTilemapData(b).layers, this.calculateIndexes()) : this.layers = [], this.currentLayer = 0, this.debugMap = [], this.dirty = !1, this._results = [], this._tempA = 0, this._tempB = 0
...
setTimeout(function () {
    game.state.add("boot", MouseGame.Boot, !0), game.state.add("preloader", MouseGame.Preloader), game.state.add("mainmenu", MouseGame.MainMenu), game.state.add("game", MouseGame.Game), game.state.add("credits", MouseGame.Credits), game.state.add("levelselector", MouseGame.LevelSelector), game.state.add("intro", MouseGame.Intro), game.state.add("ending", MouseGame.Ending)
}, 1);
var map, tileset, layer, TILE_WALLS = [201, 229, 230, 231], TILE_START = 2, TILE_GOAL = [203], TILE_SIZE = 64,
    TILE_HOLE = 211, RED_BUTTON = 84, RED_DOOR = 90, GREEN_BUTTON = 104, GREEN_DOOR = 110, YELLOW_BUTTON = 124,
    YELLOW_DOOR = 130, group, mapLayer;
MouseGame.Boot = function () {
}, MouseGame.Boot.prototype = {
    preload: function () {
        this.load.image("preloaderBar", "assets/preloader.png"), this.load.image("preloaderBarBg", "assets/preloader-bg.png"), game.load.image("preloader-background", "assets/backgrounds/preloader.jpg")
    }, create: function () {
        this.game.input.maxPointers = 1, game.stage.scaleMode = Phaser.StageScaleMode.SHOW_ALL, game.stage.scale.setShowAll(), window.addEventListener("resize", function () {
            game.stage.scale.refresh()
        }), game.stage.scale.refresh(), this.game.state.start("preloader")
    }
}, MouseGame.Credits = function (a) {
    this.game = a
}, MouseGame.Credits.prototype = {
    preload: function () {
    }, create: function () {
        this.game.add.sprite(0, 0, "credits-background");
        button = this.game.add.button(this.game.world.centerX, 630, "button-back", this.showMenu, this, 2, 1, 0), button.anchor.setTo(.5, .5)
    }, showMenu: function () {
        this.game.state.start("mainmenu")
    }
}, MouseGame.Ending = function (a) {
    this.game = a
};
var intro1, intro2, intro3, nextButton, introScreens = [];
MouseGame.Ending.prototype = {
    preload: function () {
    }, create: function () {
        title = this.game.add.sprite(0, 0, "ending"), title.inputEnabled = !0, title.events.onInputDown.add(this.showMenu, this), playMusic(!0)
    }, update: function () {
        game.input.keyboard.isDown(Phaser.Keyboard.ESC) && this.showMenu()
    }, showMenu: function () {
        this.game.state.start("mainmenu")
    }
}, MouseGame.Game = function (a) {
    this.game = a;
    {
        var a;
    ...
    levels: ["test-map", "test-scroll", "level1", "level-doors-medium", "level-doors-hard"],
    preload: function () {
        this.game.state.states.levelselector.level;
        game.load.tileset("tiles", "assets/tiles/new-tiles.png", 64, 64), game.load.spritesheet("tiles-sprites", "assets/tiles/new-tiles.png", 64, 64)
    },
    create: function () {
        createMap(), ui = game.add.group(), background = ui.add(game.add.sprite(0, 0, "background")), createButtons();
        this.game.state.states.levelselector.level;
        playMusic(!1), isPlaying = !1
    },
    update: function () {
        game.input.keyboard.isDown(Phaser.Keyboard.ESC) && this.quitToMenu()
    },
    quitToMenu: function () {
        this.game.state.start("levelselector")
    }
}, MouseGame.Intro = function (a) {
    this.game = a
};
var intro1, intro2, intro3, nextButton, introScreens = [];
